			...
			<v-card variant="outlined">
				...
				<v-row>
					<v-col v-for="wallet in walletes" :key="wallet.name">
						<v-list>
							<v-list-subheader
								><div class="text-h4">{{ wallet.name }}</div></v-list-subheader
							>
							<v-list-item v-for="address in wallet.addresses" :key="address">
								<v-list-item-header>
									<v-code class="code"> {{ address }}</v-code
									><br />
								</v-list-item-header>
							</v-list-item>
						</v-list>
				...
				<v-card-actions>
					<v-btn @click="accountModal = true" flat color="primary"
						>Edit/Add wallets</v-btn
					>
				</v-card-actions>
			</v-card>
		</v-col>
	...
	<v-overlay :model-value="accountModal" class="align-center justify-center">
		<v-card class="modal-card">
			...
			<v-card-text
				><AddWalletForm
					:handler="closeModal"
					:seedAccounts="seedAddresses"></AddWalletForm
			></v-card-text>
		</v-card>
	</v-overlay>
</template>
...
export default {
	...
	computed: {
		...
		walletes() {
			...
			for (let address of seedAddresses.filter(
